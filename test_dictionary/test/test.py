...
for _ in range(n):
    ...
    # Get the list of tuples (e.g., "1,2 3,4") from the user
    value_input = input("Enter list of tuples (e.g., '1,2 3,4'): ")
    
    # Split the input string into individual tuple strings
    tuple_strings = value_input.split()
    
    # Convert each tuple string into an actual tuple and collect them in a list
    value = [tuple(map(str, t.split(','))) for t in tuple_strings]
